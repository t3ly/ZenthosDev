	...
	if (mode == -1) {
		...
		if (status == 0) {
			if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
				...
				if (cm.getLevel() >= 30 
					&& cm.getJob().equals(Packages.client.MapleJob.THIEF)) {
			...
			if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER))
				cm.changeJob(Packages.client.MapleJob.THIEF);
			...
			if (selection == 0) {
				jobName = "Assassin";
				job = Packages.client.MapleJob.ASSASSIN;
			} else {
				jobName = "Bandit";
				job = Packages.client.MapleJob.BANDIT;
			}
