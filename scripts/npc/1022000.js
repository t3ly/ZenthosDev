	...
	if (mode == -1) {
		...
		if (status == 0) {
			if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
				...
				if (cm.getLevel() >= 30 
					&& cm.getJob().equals(Packages.client.MapleJob.WARRIOR)) {
			...
			if (selection == 0) {
				jobName = "Fighter";
				job = Packages.client.MapleJob.FIGHTER;
			} else if (selection == 1) {
				jobName = "Page";
				job = Packages.client.MapleJob.PAGE;
			} else {
				jobName = "Spearman";
				job = Packages.client.MapleJob.SPEARMAN;
			}
