...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String owner, int petid) {
        if (quantity >= 4000 || quantity < 0) {
            // AntiCheats.autoBan(c, "PE Item: " + quantity + "x " + itemId);
            return false;
        }
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        MapleInventoryType type = ii.getInventoryType(itemId);
    ...
    public static boolean addFromDrop(MapleClient c, IItem item, boolean show, String owner) {
        ...
        if (!ServerProperties.allowMoreThanOne && ii.isPickupRestricted(item.getItemId()) && c.getPlayer().haveItem(item.getItemId(), 1, true, false)) {
            c.getSession().write(MaplePacketCreator.getInventoryFull());
        ...
        if (quantity >= 4000 || quantity < 0) {
            AntiCheats.autoBan(c, "PE Item: " + quantity + "x " + item.getItemId());
            return false;
        }
        if (!type.equals(MapleInventoryType.EQUIP)) {
    ...
    public static void equip(MapleClient c, byte src, byte dst) {
        ...
        if (!c.getPlayer().isGM() && !ServerProperties.CanGMItem) {
        ...
        if (dst == -6) {
            ...
            if (top != null && InventoryConstants.isOverall(source.getItemId())) {
                if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).isFull(bottom != null && InventoryConstants.isOverall(source.getItemId()) ? 1 : 0)) {
                    c.getSession().write(MaplePacketCreator.getInventoryFull());
                    c.getSession().write(MaplePacketCreator.getShowInventoryFull());
                    return;
                }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, byte src, short quantity) {
        ...
        if (quantity < 0 || source == null || quantity == 0 && !InventoryConstants.isRechargable(itemId)) {
        ...
        if (quantity < source.getQuantity() && !InventoryConstants.isRechargable(itemId)) {
            ...
            if (weddingRing) {
                ...
                if (!ServerProperties.allowUndroppablesDrop && ii.isDropRestricted(target.getItemId())) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
                } else {
            ...
            if (c.getPlayer().getMap().getEverlast()) {
                if (!ServerProperties.allowUndroppablesDrop && ii.isDropRestricted(itemId)) {
