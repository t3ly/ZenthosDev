...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public int getDrop() {
        // System.out.println("MapleMonster#getDrop");
        MapleMonsterInformationProvider mi = MapleMonsterInformationProvider.getInstance();
        int lastAssigned = -1;
        int minChance = 1;
        List<DropEntry> dl = mi.retrieveDropChances(getId());
        // System.out.println("DropEntry size: " + dl.size());
        for (DropEntry d : dl) {
        ...
        for (DropEntry d : dl) {
            if (c >= d.assignedRangeStart && c < (d.assignedRangeStart + d.assignedRangeLength)) {
                System.out.println("Returning: " + d.itemId);
                return d.itemId;
            }
        }
        return -1;
    ...
    public MapleCharacter killBy(MapleCharacter killer) {
        long totalBaseExpL = this.getExp() * ServerProperties.getExpRate * killer.getClient().getPlayer().hasEXPCard();
