...
public class MapleLifeFactory {
    private static Map<Integer, MapleMonsterStats> monsterStats = new HashMap<Integer, MapleMonsterStats>(); // Cache
    ...
    public static MapleMonster getMonster (int mid) {
        ...
        if (stats == null) {
            final MapleData monsterData = MapleWZProvider.mobWZ.getData(StringUtil.getLeftPaddedStr(Integer.toString(mid) + ".img", '0', 11));
            if (monsterData == null) {
                return null;
            }
            ...
            stats.setName(MapleDataTool.getString(mid + "/name", MapleWZProvider.stringWZ.getData("Mob.img"), "MISSINGNO"));
            ...
            if (stats.isBoss() || mid == 8810018) {
                ...
                if (hpTagBgColor == null || hpTagColor == null) {
                    System.out.println("Monster " + stats.getName() + " (" + mid + ") flagged as boss without boss HP bars.");
                    stats.setTagColor(0);
    ...
    public static MapleNPC getNPC(int nid) {
        return new MapleNPC(nid, new MapleNPCStats(MapleDataTool.getString(nid + "/name", MapleWZProvider.stringWZ.getData("Npc.img"), "MISSINGNO")));
    }
}